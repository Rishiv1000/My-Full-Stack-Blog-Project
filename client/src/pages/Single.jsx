...
const Single = () => {
  ...
  if (!post) {
    return <div>Loading...</div>; // Show loading if post is still fetching
  }

  // Check if currentUser exists before accessing its properties
  const isCurrentUser = currentUser && currentUser.username === post.username;
...
const styles = {
  pageContainer: {
    ...
    paddingTop: '60px',  // Adjust the padding at the top
    paddingBottom: '60px',  // Correct the paddingBottom (previously paddingDown)
  },
  content: {
    width: '80%',
    maxWidth: '900px',
